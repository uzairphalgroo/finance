      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {exchanges.map((ex) => {
          ...
          return (
            <div
              key={ex.id}
              className="glass-tile card-3d rounded-2xl bg-black/90 border border-white/10 p-4.5 space-y-4 relative overflow-hidden"
            >
              {/* Header */}
              <div className="flex items-center justify-between">
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-tech font-black text-base text-white">{ex.name}</span>
                  ...
                  <p className="text-[10px] font-mono text-slate-400 mt-0.5">{ex.region}</p>
                </div>

                <div className="text-right">
                  <div className="text-[10px] font-mono text-slate-400 uppercase font-bold">Ping RTT</div>
                  <div className="text-xl font-mono-nums font-black text-white">
                    {m.pingRttMs.toFixed(1)} <span className="text-xs text-slate-400 font-normal">ms</span>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2.5 text-xs font-mono">
                {/* Jitter */}
                <div className="p-2.5 rounded-xl bg-slate-950/90 border border-white/[0.08] shadow-inner">
                  <div className="flex items-center justify-between text-[10px] text-slate-400 uppercase font-bold">
                    <span>Arrival Jitter</span>
                    <Clock className="w-3 h-3 text-slate-300" />
                  </div>
                  <div className="text-sm font-mono-nums font-black text-slate-200 mt-1">
                    {m.packetJitterMs.toFixed(2)} ms
                  </div>
                  <div className="text-[9px] text-slate-500">Welford variance</div>
                </div>
                ...
                <div className="p-2.5 rounded-xl bg-slate-950/90 border border-white/[0.08] shadow-inner">
                  ...
                  <div className="text-sm font-mono-nums font-black text-emerald-300 mt-1">
                    {m.msgPerSecond} <span className="text-[10px] text-slate-400 font-normal">msg/s</span>
                  </div>
                  <div className="text-[9px] text-slate-500">Total: {m.totalMessages.toLocaleString()}</div>
                </div>
                ...
                <div className="p-2.5 rounded-xl bg-slate-950/90 border border-white/[0.08] shadow-inner">
                  ...
                  <div className="text-[9px] text-slate-500">Exchange vs local</div>
                </div>
                ...
                <div className="p-2.5 rounded-xl bg-slate-950/90 border border-white/[0.08] shadow-inner">
                  ...
                  <div className="text-[9px] text-slate-500">Auto backoff</div>
                </div>
              </div>

              {/* Buffer Saturation Bar */}
              <div className="pt-2 border-t border-white/[0.08]">
                <div className="flex items-center justify-between text-[10px] font-mono mb-1.5 font-bold">
                  <span className="text-slate-200 flex items-center gap-1.5">
                    <Cpu className="w-3.5 h-3.5 text-slate-300" />
                    Lock-Free Ring Buffer
                  </span>
                  <span className="font-mono-nums text-slate-300">
                    {m.queueSaturationPct.toFixed(1)}% ({m.droppedMessages} dropped)
                  </span>
                </div>
